...
?>
      ...
      <td><?php echo $no;?></td>
            ...
            <td>';echo $d=$data['kode_anggota'];echo'</td>';
            $d=$data['kode_anggota'];$f=mysqli_fetch_array(mysqli_query($koneksi, "SELECT nama_anggota from t_anggota where Kode_anggota='$d'")); $rty=mysqli_fetch_array(mysqli_query($koneksi, "SELECT sum(besar_simpanan) as total_asli from t_simpan where Kode_anggota='$d'")); $inves=isset($data['besar_tabungan'])-$rty['total_asli'];?>
            <td><?php echo $f['nama_anggota'];?></td>
...
<tr  class="info"><td colspan="5"a align="center">Total</td>
  <td colspan="2">Rp. <?php $bu=mysqli_fetch_array(mysqli_query($koneksi, "SELECT sum(besar_ambil) as besar_ambil from t_pengambilan where kode_anggota='$luy'")); echo number_format(isset($bu['besar_ambil']));
  echo '</td>';?>
</tr>
