...
{ $kode=$_GET['kode_anggota']; $anggota=mysqli_fetch_array(mysqli_query($koneksi, "SELECT nama_anggota from t_anggota where kode_anggota='$kode'")); ?>
  ...
  <div class="col-lg-12">
     <div class="form-panel">
        ...
        <?php $df= mysqli_query($koneksi, "SELECT * FROM t_pinjam where kode_anggota='$kode' order by kode_pinjam desc");
			  $op=mysqli_num_rows($df);
       if (isset($op['status']) && $op['status'] == 'belum lunas')
        {
        ...
        else if (isset($op['status']) && $op['status'] == 'lunas')
        {
...
{ 
 ...
 {
	$result = mysqli_query($koneksi, "SELECT * FROM t_simpan WHERE kode_anggota='$kode' AND jenis_simpan='wajib' ORDER BY kode_simpan DESC");
	$numrow = mysqli_num_rows($result);
 	$data=isset($numrow['tgl_mulai'])?$numrow['tgl_mulai'] : '';
 	$now=date("Y-m-d");
 	if($data==$now)
 	{
